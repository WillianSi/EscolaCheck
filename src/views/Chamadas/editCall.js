import React, { useState, useEffect } from "react";
import { useNavigate, useParams } from "react-router-dom";
import { useFirebase } from "../../services/FirebaseProvider.js";
import { firestore } from "services/firebaseConfig.js";
import {
  collection,
  doc,
  getDoc,
  setDoc,
} from "firebase/firestore";

import Observ from "./observation.js";
import Excluir from "./excluir.js";
import loadingGif from "../../assets/img/brand/loading.gif";

import Auth from "layouts/Background.js";
import Header from "components/Headers/Header";
import CommonLayout from "layouts/CommonLayout.js";
import AuthenticatedLayout from "../../services/AuthenticatedLayout.js";
import useAlert from "../../hooks/useAlert.js";

import {
  Alert,
  Card,
  Container,
  Row,
  Col,
  Table,
  Button,
  Input,
} from "reactstrap";
import { BiCommentDots } from "react-icons/bi";
import { CiUser } from "react-icons/ci";
import { AiOutlineDelete } from "react-icons/ai";
import { VscSaveAs } from "react-icons/vsc";
import useFetchAlunos from "../../hooks/student.js";
import useFetchClasses from "../../hooks/class.js";

const formatTime = (date) => {
  return `${(date.getHours() < 10 ? "0" : "") + date.getHours()}:${
    (date.getMinutes() < 10 ? "0" : "") + date.getMinutes()
  }`;
};

const EditCall = () => {
  const { uid } = useFirebase();
  const { turmaId } = useParams();
  const { presencaId } = useParams();
  const { alunos } = useFetchAlunos(uid);
  const { classes } = useFetchClasses(uid);
  const [selectedId, setSelectedId] = useState(null);

  const [date, setDate] = useState(new Date().toISOString().split("T")[0]);
  const [time, setTime] = useState(formatTime(new Date()));
  const [description, setDescription] = useState("");

  const [studentNamesByIds, setStudentNamesByIds] = useState([]);
  const [selectedStudents, setSelectedStudents] = useState([]);
  const [studentsAbsentData, setStudentsAbsentData] = useState([]);

  const [observationsMap, setObservationsMap] = useState({});
  const [observModalOpen, setObservModalOpen] = useState(false);

  const [excluirModalOpen, setExcluirModalOpen] = useState(false);

  const [observation] = useState("");
  const [alunoId] = useState("");
  const navigate = useNavigate();

  const [loading, setLoading] = useState(false);
  const { errorMessage, alertColor, alertTitle, showAlert, handleAlert } =
    useAlert();

  const toggleObservModal = (alunoId) => {
    setObservModalOpen(!observModalOpen);
    setSelectedId(alunoId);
  };

  const toggleExcluirModal = (presencaId) => {
    setExcluirModalOpen(!excluirModalOpen);
    setSelectedId(presencaId);
  };

  useEffect(() => {
    const fetchData = async () => {
      try {
        if (classes && classes.length > 0 && alunos && alunos.length > 0) {
          const specificClass = classes.find((cls) => cls.id === turmaId);

          if (specificClass) {
            const studentIds = specificClass.alunos || [];

            const namesByIds = studentIds.map((studentId) => {
              const student = alunos.find((aluno) => aluno.id === studentId);
              const studentName = student
                ? student.nome
                : "Aluno cadastrado na turma não foi encontrado";
              return { id: studentId, nome: studentName };
            });
            setStudentNamesByIds(namesByIds);
          }
        }
      } catch (error) {
        handleAlert("Ao buscar dados", "danger", "Error! ");
      }
    };

    fetchData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [uid, turmaId, classes, alunos]);

  useEffect(() => {
    const loadChamadaData = async () => {
      try {
        const chamadaRef = doc(
          collection(firestore, `student/${uid}/turmas/${turmaId}/chamada`),
          presencaId
        );
        const chamadaSnapshot = await getDoc(chamadaRef);
        if (chamadaSnapshot.exists()) {
          const chamadaData = chamadaSnapshot.data();
          setDate(chamadaData.date);
          setTime(chamadaData.time);
          setDescription(chamadaData.description);
          const selectedStudentsIds = chamadaData.studentsAbsent.map(
            (student) => student.id
          );
          setSelectedStudents(selectedStudentsIds);
          const newObservationsMap = {};
          chamadaData.studentsPresent.forEach((student) => {
            newObservationsMap[student.id] = student.observation;
          });
          chamadaData.studentsAbsent.forEach((student) => {
            newObservationsMap[student.id] = student.observation;
          });
          setObservationsMap(newObservationsMap);
          const allStudentsIds = [
            ...selectedStudentsIds,
            ...chamadaData.studentsAbsent.map((student) => student.id),
          ];
          const alunoRef = doc(firestore, `student/${uid}/turmas/${turmaId}`);
          const alunoDoc = await getDoc(alunoRef);
          if (alunoDoc.exists()) {
            const alunosArray = alunoDoc.data().alunos;
            const nonMatchingIds = allStudentsIds.filter(
              (studentId) => !alunosArray.includes(studentId)
            );
            try {
              const chamadaRef = doc(
                collection(firestore, `student/${uid}/turmas/${turmaId}/chamada`),
                presencaId
              );
              const chamadaSnapshot = await getDoc(chamadaRef);
              if (chamadaSnapshot.exists()) {
                const chamadaData = chamadaSnapshot.data();
                const studentsAbsentData = [];
                for (const studentId of nonMatchingIds) {
                  const studentRef = doc(
                    collection(firestore, `student/${uid}/alunos`),
                    studentId
                  );
                  const studentSnapshot = await getDoc(studentRef);
                  if (studentSnapshot.exists()) {
                    const studentData = studentSnapshot.data();
                    const isPresent = chamadaData.studentsPresent.some(
                      (student) => student.id === studentId
                    );
                    const isAbsent = chamadaData.studentsAbsent.some(
                      (student) => student.id === studentId
                    );
                    studentsAbsentData.push({
                      id: studentId,
                      ...studentData,
                      status: isPresent
                        ? "Presente"
                        : isAbsent
                        ? "Ausente"
                        : "Não encontrado na lista de presença",
                    });
                  } else {
                    console.error(`Student with ID ${studentId} not found.`);
                  }
                }
                setStudentsAbsentData(studentsAbsentData);
              } else {
                console.error("Chamada not found.");
              }
            } catch (error) {
              console.error("Error fetching data:", error);
            }
          }
        } else {
          handleAlert("Registro não encontrado", "danger", "Erro!");
        }
      } catch (error) {
        handleAlert("Ao carregar dados", "danger", "Erro!");
      }
    };

    loadChamadaData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [uid, turmaId, presencaId]);

  const handleAdd = async () => {
    setLoading(true);
    const chamadaRef = doc(
      collection(firestore, `student/${uid}/turmas/${turmaId}/chamada`),
      presencaId
    );
    const allStudentIds = studentNamesByIds.map((aluno) => aluno.id);
    const nonSelectedStudents = allStudentIds.filter(
      (studentId) => !selectedStudents.includes(studentId)
    );
    const selectedStudentsData = studentNamesByIds.filter((aluno) =>
      selectedStudents.includes(aluno.id)
    );
    const dataToSave = {
      date: date,
      time: time,
      description: description,
      studentsAbsent: selectedStudentsData.map((aluno) => ({
        id: aluno.id,
        nome: aluno.nome,
        observation: observationsMap[aluno.id] || "",
      })),
      studentsPresent: nonSelectedStudents.map((studentId) => ({
        id: studentId,
        nome: studentNamesByIds.find((aluno) => aluno.id === studentId).nome,
        observation: observationsMap[studentId] || "",
      })),
    };
    try {
      await setDoc(chamadaRef, dataToSave, { merge: true });
      const successMessage = "Chamada editada com sucesso.";
      navigate(
        `/admin/presence/${turmaId}?message=${encodeURIComponent(
          successMessage
        )}`
      );
    } catch (e) {
      handleAlert("Ao editar chamada", "danger", "Error! ");
    } finally {
      setLoading(false);
    }
  };

  const handleCheckboxChange = (alunoId) => {
    setSelectedStudents((prevSelected) => {
      if (prevSelected.includes(alunoId)) {
        return prevSelected.filter((id) => id !== alunoId);
      } else {
        return [...prevSelected, alunoId];
      }
    });
  };

  const sortedStudentNamesByIds = [...studentNamesByIds].sort((a, b) =>
    a.nome.localeCompare(b.nome)
  );

  const sortedStudentsAbsentData = studentsAbsentData
    .filter(
      (student, index, self) =>
        index === self.findIndex((s) => s.id === student.id)
    )
    .sort((a, b) => a.nome.localeCompare(b.nome));

  return (
    <>
      <AuthenticatedLayout>
        <CommonLayout>
          <Auth corDeFundo="#485E88">
            <Header />
            <Container className="mt--7" fluid>
              <Row className="justify-content-center">
                <Col className="order-xl-1" xl="12">
                  <div
                    className="d-flex justify-content-between mb-3"
                    style={{ color: "white", fontSize: "30px" }}
                  >
                    <div className="d-flex align-items-center">
                      <span style={{ marginRight: "8px", marginLeft: "30px" }}>
                        Editar - Chamada
                      </span>
                    </div>
                    <div className="d-flex align-items-center">
                      <Button
                        color="success"
                        onClick={() => handleAdd(alunoId, observation)}
                        disabled={loading}
                        style={{
                          marginRight: "10px",
                          fontSize: "14px",
                          padding: "8px 16px",
                          zIndex: 1,
                        }}
                      >
                        {loading ? (
                          <img
                            src={loadingGif}
                            alt="Loading"
                            className="loading-imgbut"
                          />
                        ) : (
                          <VscSaveAs color="#ffff" size={20} className="mr-1" />
                        )}
                      </Button>
                      <Button
                        color="danger"
                        onClick={() => {
                          toggleExcluirModal(presencaId);
                        }}
                        style={{
                          marginRight: "10px",
                          fontSize: "14px",
                          padding: "8px 16px",
                          zIndex: 1,
                        }}
                      >
                        <AiOutlineDelete
                          color="#ffff"
                          size={20}
                          className="mr-1"
                        />
                      </Button>
                    </div>
                  </div>

                  <Card className="cardSy bg-neutral shadow">
                    <div className="floating-alert">
                      {showAlert && (
                        <Alert color={alertColor}>
                          <strong>{alertTitle}</strong> {errorMessage}
                        </Alert>
                      )}
                    </div>
                    <Row className="mb-3 mt-3">
                      <Col xs="6" className="pr-0">
                        <Input
                          id="date"
                          type="date"
                          placeholder="Data"
                          value={date}
                          onChange={(e) => setDate(e.target.value)}
                          className="inputStyle"
                        />
                      </Col>
                      <Col xs="4" className="pr-0">
                        <Input
                          id="time"
                          type="time"
                          placeholder="Hora"
                          value={time}
                          onChange={(e) => setTime(e.target.value)}
                          className="inputStyle"
                        />
                      </Col>
                      <Col xs="auto" className="pl-3">
                        <Input
                          id="description"
                          type="text"
                          placeholder="Descrição"
                          className="inputStyle"
                          value={description}
                          onChange={(e) => setDescription(e.target.value)}
                        />
                      </Col>
                    </Row>

                    <Table responsive>
                      <tbody>
                        {sortedStudentNamesByIds.map((aluno, index) => (
                          <tr key={index} className="table-row">
                            <td className="text-left">
                              <Input
                                id={`checkbox_${aluno.id}`}
                                type="checkbox"
                                checked={selectedStudents.includes(aluno.id)}
                                onChange={() => handleCheckboxChange(aluno.id)}
                              />
                              <label
                                htmlFor={`checkbox_${aluno.id}`}
                                style={{ marginLeft: "15px" }}
                              >
                                {" "}
                                <CiUser
                                  color="#5e72e4"
                                  size={20}
                                  className="mr-2"
                                />
                                {aluno.nome}
                              </label>
                            </td>
                            <td className="text-right">
                              <Button
                                className="buttonStyle"
                                onClick={() => {
                                  toggleObservModal(aluno.id);
                                }}
                              >
                                <BiCommentDots color="#5e72e4" size={25} />
                              </Button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    </Table>
                    {sortedStudentsAbsentData.length > 0 ? (
                      <Table responsive>
                        <thead>
                          <tr className="table-row">
                            <th>Aluno Remanejado</th>
                            <th>Situação</th>
                          </tr>
                        </thead>
                        <tbody>
                          {sortedStudentsAbsentData.map((student, index) => (
                            <tr
                              key={`${student.id}-${index}`}
                              className="table-row"
                            >
                              <td>{student.nome}</td>
                              <td>{student.status}</td>
                            </tr>
                          ))}
                        </tbody>
                      </Table>
                    ) : (
                      <></>
                    )}
                  </Card>
                  <div className="container mt-5"></div>
                </Col>
              </Row>
            </Container>
          </Auth>
        </CommonLayout>
        <Excluir
          isOpen={excluirModalOpen}
          toggle={toggleExcluirModal}
          presencaId={presencaId}
          turmaId={turmaId}
          uid={uid}
          handleAlert={handleAlert}
        />
        <Observ
          isOpen={observModalOpen}
          toggle={toggleObservModal}
          alunoId={selectedId}
          observationsMap={observationsMap}
          onObservationSave={(data) => {
            setObservationsMap((prevMap) => ({
              ...prevMap,
              [data.alunoId]: data.observation,
            }));
            toggleObservModal();
          }}
          uid={uid}
        />
      </AuthenticatedLayout>
    </>
  );
};

export default EditCall;